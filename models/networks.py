...
def define_UpsampleNet(gpu_ids=[], scale=2):
    ...
    if len(gpu_ids) > 0:
        net.to(gpu_ids[0])    
        if scale == 2:
            net.load_state_dict(torch.load('checkpoints/upsample_net/upsample_net_2x.pth'))
        elif scale == 4:
            net.load_state_dict(torch.load('checkpoints/upsample_net/upsample_net_4x.pth'))
        elif scale == 8:
            net.load_state_dict(torch.load('checkpoints/upsample_net/upsample_net_8x.pth'))            
...
def define_D(input_nc=3, ndf=64, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc=3, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        sequence += [nn.Sigmoid()]  # for wgangp
